...
export type OrganizationReport={
  systemScore:number;
  strengths:PositionGroup[];
  weaknesses:PositionGroup[];
  surpluses:PositionGroup[];
  blockedPlayers:{player:string;group:string;reason:string}[];
...
function groupFor(position=''){
  ...
  if(['C'].includes(p))return'Catcher';
...
function buildGroups():PositionGroup[]{
  ...
  return names.map(group=>{
    ...
    const top100=players.filter(player=>Number(player.rank||999)<=10).length;
    ...
    const depthScore=clamp(quality*.55+proximity*.25+Math.min(players.length,8)*3+top100*3-injuryCount*4);
    return{group,players:players.length,top100,averageScore:Number(quality.toFixed(1)),nearTerm,injured:injuryCount,depthScore:Number(depthScore.toFixed(1)),leaders:players.sort((a,b)=>Number(a.rank)-Number(b.rank)).slice(0,3).map(player=>player.player)};
...
function blockedPlayers(groups:PositionGroup[]){
  ...
  return rankings.filter(player=>{
    const level=statFor(player)?.level||player.level;
    return crowded.has(groupFor(player.position))&&['AAA','AA'].includes(level)&&Number(player.rank)>5;
  }).slice(0,5).map(player=>({player:player.player,group:groupFor(player.position),reason:`Near the upper minors in a crowded ${groupFor(player.position).toLowerCase()} pipeline.`}));
}
...
function recommendations(groups:PositionGroup[],blocked:{player:string;group:string;reason:string}[]){
  ...
  const output=[
    `Use ${strongest.group.toLowerCase()} depth as the system's primary trade and roster-management leverage.`,
    `Prioritize amateur and international acquisition in ${weakest?.group.toLowerCase()||'the weakest position group'}.`
  ];
...
export function isOrganizationQuestion(question:string){
  return /farm system|organization|organizational|system weakness|system strength|depth|blocked|position group|pipeline|future roster|202[7-9]|2030|assistant gm|gm briefing/i.test(question);
}
...
export function analyzeOrganization(question:string):OrganizationReport{
  ...
  const surpluses=groups.filter(group=>group.players>=6||group.top100>=3).slice(0,3);
  const blocked=blockedPlayers(groups);
  const actions=recommendations([...groups],blocked);
  const systemScore=Number((groups.reduce((sum,group)=>sum+group.depthScore,0)/groups.length).toFixed(1));
  const focus=/weak|need|lack|thin/i.test(question)?weaknesses:/surplus|trade|blocked/i.test(question)?surpluses:strengths;
  const lines=focus.map((group,index)=>`${index+1}. ${group.group} — depth score ${group.depthScore}/100, ${group.players} tracked prospects, ${group.nearTerm} at Double-A or Triple-A. Leaders: ${group.leaders.join(', ')}.`);
  const answer=`The organizational model rates the Phillies farm system at ${systemScore}/100.\n\n${/weak|need|lack|thin/i.test(question)?'Biggest weaknesses':/surplus|trade|blocked/i.test(question)?'Most usable surpluses':'Biggest strengths'}:\n${lines.join('\n')}\n\nRecommended actions:\n${actions.map((action,index)=>`${index+1}. ${action}`).join('\n')}${blocked.length?`\n\nPotentially blocked players:\n${blocked.map(player=>`- ${player.player}: ${player.reason}`).join('\n')}`:''}\n\nThis is an internal organizational model based on the currently tracked prospect pool, level, rank, performance score and injury evidence—not a complete MLB 40-man roster model yet.`;
