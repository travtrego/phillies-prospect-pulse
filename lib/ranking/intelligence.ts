...
const component=(row:Row,key:string,fallback=50)=>Number.isFinite(Number(row.components?.[key]))?clamp(Number(row.components[key])/(key==='risk'?5:key==='movement'||key==='ageLevel'?10:key==='performance'?25:key==='sentiment'?20:30)*100):fallback;
...
function developmentSignal(row:Row){
  const movement=Number(row.change||0);
  return clamp(50+movement*7);
}

export function rankingIntelligence(row:Row):RankingIntelligence{
  ...
  const evaluation=evaluateRanking({
    scouting:component(row,'scouting'),
    performance:component(row,'performance'),
    ageLevel:component(row,'ageLevel'),
    sentiment:component(row,'sentiment'),
    movement:component(row,'movement'),
    risk:100-component(row,'risk'),
    development,
    defense:defense??undefined,
    pitchQuality:pitchQuality??undefined,
    history:Number.isFinite(Number(row.historicalScore))?Number(row.historicalScore):undefined,
...
export function enrichRankings(){return rankings.map(row=>({...row,intelligence:rankingIntelligence(row)}));}
