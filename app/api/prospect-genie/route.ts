...
import { buildPredictiveFrontOfficeReport, isPredictiveFrontOfficeQuestion } from '../../../lib/genie/predictiveFrontOffice';
...
export async function POST(request:NextRequest){
  try{
    ...
    if(isPredictiveFrontOfficeQuestion(question)){
      const predictiveFrontOffice=await buildPredictiveFrontOfficeReport(question,matchedPlayers);
      return respond({
        engine:'Prospect Genie Predictive Front Office v10.0',
        answer:predictiveFrontOffice.answer,
        matchedPlayers,
        intent:{task:'predictive_front_office',metric:'future_roster_value',filters:{},limit:26},
        plan:[
          {tool:'loadCurrentPhilliesRoster',description:'Load the current active and 40-man Phillies rosters from the MLB Stats API when available.'},
          {tool:'projectProspectArrivals',description:'Estimate prospect arrival seasons from canonical rank, level, readiness, health and development evidence.'},
          {tool:'constructFuture26ManRosters',description:'Build one-, two- and three-season roster scenarios by position group.'},
          {tool:'modelFortyManAndRule5',description:'Estimate protection pressure and Rule 5 decisions from public eligibility evidence.'},
          {tool:'detectFutureRosterCrunches',description:'Identify position groups where projected depth exceeds likely roster capacity.'},
          {tool:'simulateTradePackage',description:'Estimate package value and target-tier fit when the question includes a trade scenario.'},
          {tool:'forecastPromotionImpact',description:'Measure how prospect arrivals change future depth and acquisition needs.'},
          {tool:'auditAnswerReliability',description:'Run the internal consistency and source-freshness guardrail.'}
        ],
        confidence:predictiveFrontOffice.confidence,
        limitations:predictiveFrontOffice.limitations,
