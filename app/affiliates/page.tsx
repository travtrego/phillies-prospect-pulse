 ...
 return <main>
  ...
  <section className="affiliateGrid">{levels.map(([level,team,label])=>{
   ...
   return <article className="affiliateCard" key={level}>
    ...
    <div className="affiliateMetrics"><div><span>Active roster</span><strong>{roster.length}</strong></div><div><span>Ranked</span><strong>{ranked.length}</strong></div><div><span>Injury notes</span><strong>{teamInjuries.length}</strong></div></div>
