...
export default function ProspectDirectory({players,rankings}:{players:Player[];rankings:HomepageRanking[]}){
 ...
 return <>
  <section className="directoryToolbar"><div className="filterTabs" role="tablist" aria-label="Filter prospects">{filters.map(([value,label])=><button type="button" role="tab" aria-selected={filter===value} key={value} className={filter===value?'active':''} onClick={()=>setFilter(value)}>{label}</button>)}</div><input aria-label="Search players" placeholder="Search player, team or position…" value={search} onChange={event=>setSearch(event.target.value)}/></section>
  ...
  <section className="cardGrid" aria-live="polite">{visiblePlayers.map(player=>{const displayRank=getRank(player);const bio=[player.current_age?`Age ${player.current_age}`:null,player.height,player.weight?`${player.weight} lb`:null].filter(Boolean).join(' · ');return <article className="playerCard" key={player.id}>
   ...
   <Link className="statsLink" href={`/players/${player.id}`} aria-label={`Open stats and full profile for ${player.full_name}`}>Stats & Full Profile →</Link><footer className="cardFooter"><span>{player.source_name}</span><span>Checked {formatDate(player.source_last_verified_at)}</span></footer>
