...
function cleanSummary(summary: string, source: string, title: string) {
  let cleaned = summary.replace(/&nbsp;/gi, ' ').replace(/&amp;/gi, '&').replace(/&#39;/gi, "'").replace(/&quot;/gi, '"').replace(/\s+/g, ' ').trim();
  if (cleaned.toLowerCase().endsWith(source.toLowerCase())) cleaned = cleaned.slice(0, -source.length).trim();
  return !cleaned || cleaned.toLowerCase() === title.toLowerCase() ? 'Open the full story for details.' : cleaned;
}
