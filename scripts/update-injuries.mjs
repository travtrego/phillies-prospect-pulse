...
const injuryPatterns = [
  [/(tommy john|ucl|elbow)/i, "Elbow/UCL"],
  [/(shoulder|rotator cuff|labrum)/i, "Shoulder"],
  [/(hamstring)/i, "Hamstring"],
  [/(oblique)/i, "Oblique"],
  [/(back|spine)/i, "Back"],
  [/(wrist)/i, "Wrist"],
  [/(hand|finger|thumb)/i, "Hand/Finger"],
  [/(ankle)/i, "Ankle"],
  [/(knee|acl|mcl|meniscus)/i, "Knee"],
  [/(foot|toe)/i, "Foot/Toe"],
  [/(hip|groin)/i, "Hip/Groin"],
  [/(concussion|head)/i, "Concussion/Head"],
  [/(illness|sick)/i, "Illness"]
];
...
async function findNewsInjury(record) {
  const query = `\"${record.player}\" \"${record.affiliate}\" injury`;
  ...
  try {
    ...
    for (const item of items.slice(0, 10)) {
      ...
      if (injury && combined.toLowerCase().includes(record.player.toLowerCase())) {
